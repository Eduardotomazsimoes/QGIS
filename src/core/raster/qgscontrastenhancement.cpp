  ...
  ,   mLookupTableOffset( mMinimumValue * -1 )
{
  ...
  if ( mRasterDataTypeRange <= 65535.0 )
  {
    ...
    mDisplayableLookupTable = new bool[static_cast <int>( mRasterDataTypeRange + 1 )];
  }
}
...
{
  delete [] mLookupTable;
  delete [] mDisplayableLookupTable;
}
...
{
  if ( mLookupTable && !mEnhancementDirty )
  {
    double shiftedValue = value + mLookupTableOffset;
    if ( shiftedValue >= 0 && shiftedValue < mRasterDataTypeRange + 1 )
      return mDisplayableLookupTable[static_cast <int>( shiftedValue )];
    return false;
  }
